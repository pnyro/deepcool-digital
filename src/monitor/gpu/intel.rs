//! Reads live GPU data from Intel GPUs.
...
mod platform {
    /// Intel GPU monitoring is not supported on Windows.
    pub struct Gpu;

    impl Gpu {
        pub fn new(_pci_address: &str) -> Option<Self> {
            None
        }

        // Stubs required for compilation — never called since new() returns None
        pub fn get_temp(&self, _fahrenheit: bool) -> u8 { 0 }
        pub fn get_usage(&self) -> u8 { 0 }
        pub fn get_power(&self) -> u16 { 0 }
        pub fn get_frequency(&self) -> u16 { 0 }
    }
}
